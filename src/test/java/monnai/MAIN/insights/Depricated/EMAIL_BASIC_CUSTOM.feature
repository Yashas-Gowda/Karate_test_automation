...
Feature: Testing of DPI  - EMAIL_BASIC_CUSTOM feature scenarios with data partner WhoIsXML, ZeroBounce,AtData, HaveIBeenPawned.
  ...
  Background:
    ...
    * path '/api/insights/'
    #    * def Custom_env_authFeature = call read('classpath:monnai/Auth_Token_Generation_from_Custom_Tenant.feature') { _Custom_env : email_custom_tenant }
    #    * def BearerToken = Custom_env_authFeature.authToken
    #    * def  Custom_env_authFeature_tenant_config = Custom_env_authFeature.Auth_custom_tenant_config
